...
7. Users must be able to connect to the server using Jabber client (choose jabber protocol support level you can afford)
8. Servers must be able to support federation i.e. messages between servers (this will require more complicated setup in docker compose)
9. Use jabber library available for your tech stack.
   Most advanced possible implementation must include load test for federation of two servers:
10. 50+ clients connected to one server - A, 50+ to another - B
11. Messaging from A to B and back.
...
12. Connection dashboard for admin
13. Federation traffic info/statistics
14. How to submit HW results
