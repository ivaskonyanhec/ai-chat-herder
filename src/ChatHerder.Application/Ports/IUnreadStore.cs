...
public interface IUnreadStore
{
    ...
    Task<IReadOnlyList<(string ContextType, Guid ContextId, long Count)>> GetAllAsync(Guid userId, IReadOnlyList<(string ContextType, Guid ContextId)> contexts, CancellationToken ct = default);
}
